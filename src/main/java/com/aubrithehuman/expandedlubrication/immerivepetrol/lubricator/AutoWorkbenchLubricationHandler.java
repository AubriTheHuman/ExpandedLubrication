...
//	private static final float factor = AMIConfig.tier_1_factor.get();
	...
	@Override
	public AutoLubricatorTileEntity getLubricatorTE(World world, AutoWorkbenchTileEntity mbte) {
		if(!world.isClientSide){
			
			TileEntity lubricator = null;
			
			if(!mbte.isDummy()){
				BlockPos posLube = mbte.getBlockPos().relative(mbte.getFacing().getCounterClockWise(), 2).below();
	...
	@Override
	public void lubricateWithFluid(World world, int ticks, Fluid fluid, AutoWorkbenchTileEntity mbte) {
...
//			mbte.animation_agitator %= 360f;
	...
	@Override
	public void spawnLubricantParticles(World world, AutoLubricatorTileEntity lubricator, Direction facing, AutoWorkbenchTileEntity mbte){
