...
public class AlloySmelterLubricationHandler implements ILubricationHandler<AlloySmelterTileEntity>{
//	private static final float factor = AMIConfig.tier_1_factor.get();
	...
	public TileEntity isPlacedCorrectly(World world, AutoLubricatorTileEntity lubricator, Direction facing){
		...
		if(te instanceof AlloySmelterTileEntity){
			AlloySmelterTileEntity master = ((AlloySmelterTileEntity) te).master();
	...
	@Override
	public void lubricate(World arg0, int arg1, AlloySmelterTileEntity arg2) { }

	@Override
	public void lubricate(World arg0, Fluid arg1, int arg2, AlloySmelterTileEntity arg3) { }
	
	@Override
	public void lubricateClient(ClientWorld arg0, Fluid arg1, int arg2, AlloySmelterTileEntity arg3) {
		
	}

	@Override
	public void lubricateServer(ServerWorld world, Fluid lubricant, int ticks, AlloySmelterTileEntity mbte) {
		tierProcess(lubricant, mbte, ticks, world);		
	}
	
	public void tierProcess(Fluid fluid, AlloySmelterTileEntity mbte, int ticks, World world) {
		double times = 0.0D;
...
//		System.out.println("doing " + times);
	...
	public boolean process(AlloySmelterTileEntity mbte, int ticks, World world) {
		mbte.tick();
		return true;
	}
	
	@Override
	public boolean isMachineEnabled(World world, AlloySmelterTileEntity mbte){
		return mbte.getIsActive();
	}
				
	@Override
	public void spawnLubricantParticles(ClientWorld world, AutoLubricatorTileEntity lubricator, Direction facing, AlloySmelterTileEntity mbte){
	...
	@Override
	public Tuple<BlockPos, Direction> getGhostBlockPosition(World world, AlloySmelterTileEntity mbte){
//		System.out.println("checkghost");
		if(!mbte.isDummy()){
			BlockPos pos = mbte.getBlockPos().relative(mbte.getFacing().getClockWise(), 1).above();
	...
	@Override
	@OnlyIn(Dist.CLIENT)
	public void renderPipes(AutoLubricatorTileEntity lubricator, AlloySmelterTileEntity mbte, MatrixStack matrix, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay){
