...
public class HydroTreaterLubricationHandler extends TieredLubricationHandler<AutoLubricatorTileEntity, HydrotreaterTileEntity, SulfurRecoveryRecipe>{
//	private static final float factor = AMIConfig.tier_1_factor.get();
	private static Vector3i size = new Vector3i(3, 3, 4);
...
//		System.out.println("placed correctly check");
		if(te instanceof HydrotreaterTileEntity){
			HydrotreaterTileEntity master = ((HydrotreaterTileEntity) te).master();
	...
	@Override
	public AutoLubricatorTileEntity getLubricatorTE(World world, HydrotreaterTileEntity mbte) {
...
//				System.out.println(posLube);
	...
	@Override
	public void lubricateWithFluid(World world, int ticks, Fluid fluid, HydrotreaterTileEntity mbte) {
//		AMICore.LOGGER.info(factor);
		if(!world.isClientSide){	
//			Iterator<MultiblockProcess<SulfurRecoveryRecipe>> processIterator = mbte.processQueue.iterator();
//			System.out.println("success");
//			if (!processIterator.hasNext()) return;
//			System.out.println("success2");
//			MultiblockProcess<SulfurRecoveryRecipe> process = processIterator.next();
			if(fluid != null)
			tierProcess(fluid, null, null, mbte, ticks);
		}else{
...
//			mbte.animation_agitator %= 360f;
	...
	@Override
	public boolean process(Iterator<MultiblockProcess<SulfurRecoveryRecipe>> processIterator, MultiblockProcess<SulfurRecoveryRecipe> process, HydrotreaterTileEntity mbte) {
		mbte.tick();
...
//		}
	...
	@Override
	public void spawnLubricantParticles(World world, AutoLubricatorTileEntity lubricator, Direction facing, HydrotreaterTileEntity mbte){
	...
	@Override
	public Tuple<BlockPos, Direction> getGhostBlockPosition(World world, HydrotreaterTileEntity mbte){
//		System.out.println("checkghost");
	...
	@Override
	@OnlyIn(Dist.CLIENT)
	public void renderPipes(AutoLubricatorTileEntity lubricator, HydrotreaterTileEntity mbte, MatrixStack matrix, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay){
...
//				matrix.rotate(new Quaternion(0, 180F, 0, true));
	...
	@Override
	public boolean isMachineEnabled(World world, HydrotreaterTileEntity mbte){
