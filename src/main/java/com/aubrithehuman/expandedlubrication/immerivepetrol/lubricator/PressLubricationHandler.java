...
public class PressLubricationHandler extends TieredLubricationHandler<AutoLubricatorTileEntity, MetalPressTileEntity, MetalPressRecipe>{
//	private static final float factor = AMIConfig.tier_1_factor.get();
	private static Vector3i size = new Vector3i(1, 2, 3);
	...
	public TileEntity isPlacedCorrectly(World world, AutoLubricatorTileEntity lubricator, Direction facing){
		...
		if(te instanceof MetalPressTileEntity){
			MetalPressTileEntity master = ((MetalPressTileEntity) te).master();
	...
	@Override
	public boolean process(Iterator<MultiblockProcess<MetalPressRecipe>> processIterator, MultiblockProcess<MetalPressRecipe> process, MetalPressTileEntity mbte, int ticks, World world) {
		mbte.tick();
...
//		}
	...
	@Override
	public AutoLubricatorTileEntity getLubricatorTE(World world, MetalPressTileEntity mbte) {
		if(!world.isClientSide){
			
			TileEntity lubricator = null;
			
			if(!mbte.isDummy()){
				BlockPos posLube = mbte.getBlockPos().relative(mbte.getFacing(), 1).relative(mbte.getFacing().getCounterClockWise(), 1).below();
	...
	@Override
	public void lubricateWithFluid(World world, int ticks, Fluid fluid, MetalPressTileEntity mbte) {
//		AMICore.LOGGER.info(factor);
		if(!world.isClientSide){	
			Iterator<MultiblockProcess<MetalPressRecipe>> processIterator = mbte.processQueue.iterator();
			MultiblockProcess<MetalPressRecipe> process = processIterator.next();
...
//			mbte.animation_agitator %= 360f;
	...
	@Override
	public void spawnLubricantParticles(World world, AutoLubricatorTileEntity lubricator, Direction facing, MetalPressTileEntity mbte){
	...
	@Override
	public Tuple<BlockPos, Direction> getGhostBlockPosition(World world, MetalPressTileEntity mbte){
//		System.out.println("checkghost");
		if(!mbte.isDummy()){
			BlockPos pos = mbte.getBlockPos().relative(mbte.getFacing(), 1).relative(mbte.getFacing().getCounterClockWise(), 1);
	...
	@Override
	@OnlyIn(Dist.CLIENT)
	public void renderPipes(AutoLubricatorTileEntity lubricator, MetalPressTileEntity mbte, MatrixStack matrix, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay){
