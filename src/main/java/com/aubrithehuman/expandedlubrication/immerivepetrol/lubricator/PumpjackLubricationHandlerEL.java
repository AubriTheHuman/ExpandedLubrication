...
//	private static final float factor = AMIConfig.tier_1_factor.get();
	...
	@Override
	public AutoLubricatorTileEntity getLubricatorTE(World world, PumpjackTileEntity mbte) {
		if(!world.isClientSide){
			
			TileEntity lubricator = null;
			if(!mbte.isDummy()){
				Direction mbFacing = mbte.getFacing().getOpposite();
				BlockPos posLube = mbte.getBlockPos()
					.relative(Direction.UP)
					.relative(mbFacing, 4)
					.relative(mbte.getIsMirrored() ? mbFacing.getClockWise() : mbFacing.getCounterClockWise(), 2).below();
				lubricator = world.getBlockEntity(posLube);
//				System.out.println(posLube.toString());
				if(lubricator != null) {
	...
	@Override
	public void lubricateWithFluid(World world, int ticks, Fluid fluid, PumpjackTileEntity mbte) {
