...
//		System.out.println("placed correctly check");
	...
	@Override
	public AutoLubricatorTileEntity getLubricatorTE(World world, CokerUnitTileEntity mbte) {
		if(!world.isClientSide){
			
			TileEntity lubricator = null;
//			System.out.println(lubricator);
			if(!mbte.isDummy()){
				BlockPos posLube = mbte.getBlockPos().relative(mbte.getFacing().getOpposite(), 3).relative(Direction.UP, 1).below();
//				System.out.println(posLube);
	...
	@Override
	public void lubricateWithFluid(World world, int ticks, Fluid fluid, CokerUnitTileEntity mbte) {
//		AMICore.LOGGER.info(factor);
		if(!world.isClientSide){	
//			Iterator<MultiblockProcess<CokerUnitRecipe>> processIterator = mbte.processQueue.iterator();
//			if (!processIterator.hasNext()) return;
//			MultiblockProcess<CokerUnitRecipe> process = processIterator.next();
			if(fluid != null)
...
//			mbte.animation_agitator %= 360f;
	...
	@Override
	public boolean process(Iterator<MultiblockProcess<CokerUnitRecipe>> processIterator, MultiblockProcess<CokerUnitRecipe> process, CokerUnitTileEntity mbte) {
		mbte.tick();
...
//		}
	...
	@Override
	public void spawnLubricantParticles(World world, AutoLubricatorTileEntity lubricator, Direction facing, CokerUnitTileEntity mbte){
