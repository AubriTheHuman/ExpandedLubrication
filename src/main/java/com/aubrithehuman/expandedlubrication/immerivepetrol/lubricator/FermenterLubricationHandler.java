...
public class FermenterLubricationHandler extends TieredLubricationHandler<AutoLubricatorTileEntity, FermenterTileEntity, FermenterRecipe>{
//	private static final float factor = AMIConfig.tier_1_factor.get();
	...
	public TileEntity isPlacedCorrectly(World world, AutoLubricatorTileEntity lubricator, Direction facing){
		...
		if(te instanceof FermenterTileEntity){
			FermenterTileEntity master = ((FermenterTileEntity) te).master();
	...
	@Override
	public boolean process(Iterator<MultiblockProcess<FermenterRecipe>> processIterator, MultiblockProcess<FermenterRecipe> process, FermenterTileEntity mbte, int ticks, World world) {
		mbte.tick();
...
//		}
	...
	@Override
	public AutoLubricatorTileEntity getLubricatorTE(World world, FermenterTileEntity mbte) {
		if(!world.isClientSide){
			
			TileEntity lubricator = null;
			
			if(!mbte.isDummy()){
				BlockPos posLube = mbte.getBlockPos().relative(mbte.getFacing(), 2).below();
	...
	@Override
	public void lubricateWithFluid(World world, int ticks, Fluid fluid, FermenterTileEntity mbte) {
//		AMICore.LOGGER.info(factor);
		if(!world.isClientSide){	
			Iterator<MultiblockProcess<FermenterRecipe>> processIterator = mbte.processQueue.iterator();
			MultiblockProcess<FermenterRecipe> process = processIterator.next();
...
//			mbte.animation_agitator %= 360f;
	...
	@Override
	public void spawnLubricantParticles(World world, AutoLubricatorTileEntity lubricator, Direction facing, FermenterTileEntity mbte){
	...
	@Override
	public Tuple<BlockPos, Direction> getGhostBlockPosition(World world, FermenterTileEntity mbte){
//		System.out.println("checkghost");
	...
	@Override
	@OnlyIn(Dist.CLIENT)
	public void renderPipes(AutoLubricatorTileEntity lubricator, FermenterTileEntity mbte, MatrixStack matrix, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay){
