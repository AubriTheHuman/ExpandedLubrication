...
public class ArcFurnaceLubricationHandler extends TieredLubricationHandler<AutoLubricatorTileEntity, ArcFurnaceTileEntity, ArcFurnaceRecipe>{
//	private static final float factor = AMIConfig.tier_1_factor.get();
	...
	public TileEntity isPlacedCorrectly(World world, AutoLubricatorTileEntity lubricator, Direction facing){
		...
		if(te instanceof ArcFurnaceTileEntity){
			ArcFurnaceTileEntity master = ((ArcFurnaceTileEntity) te).master();
	...
	@Override
	public AutoLubricatorTileEntity getLubricatorTE(World world, ArcFurnaceTileEntity mbte) {
		if(!world.isClientSide){
			
			TileEntity lubricator = null;
			
			if(!mbte.isDummy()){
				BlockPos posLube = mbte.getBlockPos().relative(mbte.getFacing().getClockWise(), 3).below();
	...
	@Override
	public void lubricateWithFluid(World world, int ticks, Fluid fluid, ArcFurnaceTileEntity mbte) {
//		AMICore.LOGGER.info(factor);
		if(!world.isClientSide){	
//			System.out.println("Attempt Process");
			Iterator<MultiblockProcess<ArcFurnaceRecipe>> processIterator = mbte.processQueue.iterator();
			MultiblockProcess<ArcFurnaceRecipe> process = processIterator.next();
...
//			mbte.anim %= 360f;
	...
	@Override
	public void spawnLubricantParticles(World world, AutoLubricatorTileEntity lubricator, Direction facing, ArcFurnaceTileEntity mbte){
	...
	@Override
	public Tuple<BlockPos, Direction> getGhostBlockPosition(World world, ArcFurnaceTileEntity mbte){
//		System.out.println("checkghost");
	...
	@Override
	@OnlyIn(Dist.CLIENT)
	public void renderPipes(AutoLubricatorTileEntity lubricator, ArcFurnaceTileEntity mbte, MatrixStack matrix, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay){
