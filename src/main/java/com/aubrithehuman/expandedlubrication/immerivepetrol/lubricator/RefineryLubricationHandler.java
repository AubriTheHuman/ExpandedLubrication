...
public class RefineryLubricationHandler extends TieredLubricationHandler<AutoLubricatorTileEntity, RefineryTileEntity, RefineryRecipe>{
//	private static final float factor = AMIConfig.tier_1_factor.get();
	private static Vector3i size = new Vector3i(5, 3, 3);
	...
	public TileEntity isPlacedCorrectly(World world, AutoLubricatorTileEntity lubricator, Direction facing){
		...
		if(te instanceof RefineryTileEntity){
			RefineryTileEntity master = ((RefineryTileEntity) te).master();
	...
	@Override
	public AutoLubricatorTileEntity getLubricatorTE(World world, RefineryTileEntity mbte) {
		if(!world.isClientSide){
			
			TileEntity lubricator = null;
			
			if(!mbte.isDummy()){
				BlockPos posLube = mbte.getBlockPos().relative(mbte.getFacing(), 3).below();
	...
	@Override
	public boolean process(Iterator<MultiblockProcess<RefineryRecipe>> processIterator, MultiblockProcess<RefineryRecipe> process, RefineryTileEntity mbte, int ticks, World world) {
		mbte.tick();
...
//		}
	...
	@Override
	public void lubricateWithFluid(World world, int ticks, Fluid fluid, RefineryTileEntity mbte) {
//		AMICore.LOGGER.info(factor);
		if(!world.isClientSide){	
			Iterator<MultiblockProcess<RefineryRecipe>> processIterator = mbte.processQueue.iterator();
			MultiblockProcess<RefineryRecipe> process = processIterator.next();
...
//			mbte.animation_agitator %= 360f;
	...
	@Override
	public void spawnLubricantParticles(World world, AutoLubricatorTileEntity lubricator, Direction facing, RefineryTileEntity mbte){
	...
	@Override
	public Tuple<BlockPos, Direction> getGhostBlockPosition(World world, RefineryTileEntity mbte){
//		System.out.println("checkghost");
		if(!mbte.isDummy()){
			BlockPos pos = mbte.getBlockPos().relative(mbte.getFacing(), 3);
	...
	@Override
	@OnlyIn(Dist.CLIENT)
	public void renderPipes(AutoLubricatorTileEntity lubricator, RefineryTileEntity mbte, MatrixStack matrix, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay){
